...
function buildSidebarHTML() {
  ...
  function moduleEntry(mod) {
    ...
    else if (active)    badge = `<span class="sb-module-status sb-status-active">Active</span>`;
    else if (!unlocked) badge = `<span class="sb-module-status sb-status-locked">Locked</span>`;
      ...
      */
      const items = steps
        .map((s, i) => {
          ...
          if (done) {
            return `<button class="${cls}" data-jump="${i}"
