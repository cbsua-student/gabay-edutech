  ...
  */
  ...
  sims: {
    'Physics': [
      ...
      { name: 'SimuPhysics',       desc: 'Mechanics, waves, optics, and electricity simulations', url: 'https://simuphysics.com' },
      { name: 'Molecular Workbench', desc: 'Physics, chemistry, and biology molecular models',   url: 'https://mw.concord.org' },
      { name: 'Labster',           desc: 'Immersive virtual physics lab simulations',             url: 'https://www.labster.com' },
      { name: 'JavaLab',           desc: 'Free HTML5 science simulations, including physics',     url: 'https://javalab.org' },
      { name: 'A Free Particle',   desc: 'Introductory mechanics simulations for intro physics',  url: 'https://www.afreeparticle.com' },
    ],
    'Chemistry': [
      ...
      { name: 'Molecular Workbench', desc: 'Molecular-level chemistry simulations',             url: 'https://mw.concord.org' },
      { name: 'Labster',           desc: 'Immersive virtual chemistry lab simulations',          url: 'https://www.labster.com' },
      { name: 'VirtualChem Labs',  desc: 'Computational chemistry and virtual lab tools',        url: 'https://virtualchemlabs.com' },
      { name: 'NMSU Virtual Labs', desc: 'Lab skills, pH, and chemistry procedures',             url: 'https://virtuallabs.nmsu.edu' },
      { name: 'JavaLab',           desc: 'Free HTML5 science simulations, including chemistry',  url: 'https://javalab.org' },
    ],
    'Biology': [
      ...
      { name: 'Labster',             desc: 'Immersive biology and health science lab simulations', url: 'https://www.labster.com' },
      { name: 'Biology Simulations', desc: 'Data-analysis focused biology experiments',        url: 'https://www.biologysimulations.com' },
      { name: 'MERLOT Biology Labs', desc: 'Inquiry-based AP and college biology labs',        url: 'https://virtuallabs.merlot.org' },
      { name: 'NMSU Virtual Labs',   desc: 'Microscope, bacteria, and food-safety labs',       url: 'https://virtuallabs.nmsu.edu' },
      { name: 'PraxiLabs',           desc: 'Virtual biology laboratory simulations',           url: 'https://praxilabs.com' },
      { name: 'BioNetwork',          desc: 'Manipulate a virtual microscope',                  url: 'https://www.ncbionetwork.org' },
    ],
    'Earth Science': [
      ...
      { name: 'MERLOT Earth Science', desc: 'Global warming and environmental simulations',   url: 'https://virtuallabs.merlot.org' },
      { name: 'ExploreLearning Gizmos', desc: 'Plate tectonics, greenhouse effect, and Earth labs', url: 'https://gizmos.explorelearning.com' },
      { name: 'Layered Earth',      desc: '3D geology, meteorology, and climate models',      url: 'https://www.layeredearth.com' },
      { name: 'SEPUP Digital Activities', desc: 'Earthquakes, volcanoes, and plate boundaries', url: 'https://sepup.lawrencehallofscience.org' },
      { name: 'EduMedia',           desc: 'Interactive Earth geodynamics simulations',        url: 'https://www.edumedia.com' },
    ],
  },
};
...
/* Jump directly to a step by index (review mode, via sidebar accordion) */
function jumpToStep(step) {
  if (!state.course) return;
  state.course.step = step;
  saveStep(state.course.id, step);
  renderStep();
}
...
function buildSidebarHTML() {
  ...
  /* ================================================================
     7a. CURRENT-LOCATION ICON
     Small pin icon reused everywhere the sidebar needs to say
     "you are here" (module badge + step marker), so the cue is a
     shape, not just a color, and stays consistent across levels.
     ================================================================ */
  const ICON_HERE = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
       stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    <path d="M21 10c0 6.5-9 12-9 12s-9-5.5-9-12a9 9 0 0 1 18 0z"/>
    <circle cx="12" cy="10" r="2.8"/>
  </svg>`;

  const ICON_LOCKED = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
       stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
  ...
  function moduleEntry(mod) {
    ...
    /* Status badge */
    let badge = '';
    if      (done)      badge = `<span class="sb-module-status sb-status-done">Done</span>`;
    else if (active)    badge = `<span class="sb-module-status sb-status-active">${ICON_HERE} Active</span>`;
    else if (!unlocked) badge = `<span class="sb-module-status sb-status-locked">${ICON_LOCKED} Locked</span>`;
    ...
    */
      ...
      */
      const items = steps
        .map((s, i) => {
          ...
          /* Marker: pin icon on the current step, plain dot otherwise */
          const marker  = isCurrent ? ICON_HERE : `<span class="sb-step-dot"></span>`;
          const curAttr = isCurrent ? ' aria-current="step"' : '';

          /* In review mode: clickable buttons. First pass: static spans. */
          if (done) {
            return `<button class="${cls}" data-jump="${i}"${curAttr}
                            aria-label="Go to: ${esc(label)}">
                      ${marker}${esc(label)}
                    </button>`;
          }
          return `<span class="${cls}"${curAttr}>
                    ${marker}${esc(label)}
                  </span>`;
        })
    ...
    return `
      <div class="sb-module">
        <button class="${hdrClass}" ${attrs} aria-label="${esc(mod.title)}">
          ${esc(mod.title)} ${badge}
        </button>
        ${accordion}
      </div>`;
...
function renderSidebar() {
  ...
  $('sidebar-nav').querySelectorAll('[data-jump]').forEach(btn =>
    btn.addEventListener('click', () => jumpToStep(Number(btn.dataset.jump))));
  ...
  return `
    <div class="about-screen">
      ...
      <div class="team-grid" style="margin-top:0.9rem">
        <div class="team-card">
          <p class="tc-name">John Napoleon G. Flororita</p>
...
function showLogin() {
  ...
  /* Its target (#main-content) lives inside app-wrapper, so it has
     nothing to jump to until the person is logged in. */
...
$('logout-btn').addEventListener('click', () => {
  /*
    Wipe everything on logout: session, progress flags, workspace
    content, journal answers, and ratings. This is intentional for
    the single-user prototype -- once a server/account system is in
    place, saved work will live server-side instead and this can go
    back to only clearing the session token.
  */
  store.remove('user');
  store.remove('prog');
  store.remove('ws');
  store.remove('journal');
  store.remove('ratings');
  state.course = null;
  state.view   = 'dashboard';
  closeSidebar();
  showLogin();
